...
function makeMockEnv(): import('../src/index').Env {
	return {
...
describe('Form submission worker', () => {
	...
	it('rejects missing required fields', async () => {
		...
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(partial) });
	...
	it('rejects invalid email', async () => {
		...
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(bad) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('rejects future date of birth', async () => {
		...
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(future) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('requires parent fields for minors', async () => {
		...
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(minor) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('rejects oversized participant signature', async () => {
		const big = { ...validSubmission, signatureParticipant: `data:image/png;base64,${bigBase64}` };
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(big) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('accepts valid adult submission', async () => {
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(validSubmission) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 200) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('accepts empty optional fields for adults', async () => {
		const req = new Request('http://test', {
			method: 'POST',
			body: JSON.stringify({ ...validSubmission, fullNameParent: '', signatureParent: '' }),
		});
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 200) {
			const text = await res.text();
	...
	it('accepts text signatureParticipant', async () => {
		const textSig = { ...validSubmission, signatureParticipant: 'John Doe' };
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(textSig) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 200) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('rejects invalid base64 with PNG prefix', async () => {
		const bad = { ...validSubmission, signatureParticipant: 'data:image/png;base64,not-valid-base64' };
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify(bad) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
	...
	it('accepts non-PNG signature (jpeg)', async () => {
		...
		const req = new Request('http://test', {
			method: 'POST',
			body: JSON.stringify({ ...validSubmission, signatureParticipant: `data:image/jpeg;base64,${jpegBase64}` }),
		});
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 200) {
			const text = await res.text();
	...
	it('accepts duplicate submission', async () => {
		const req1 = new Request('http://test', { method: 'POST', body: JSON.stringify(validSubmission) });
		const req2 = new Request('http://test', { method: 'POST', body: JSON.stringify(validSubmission) });
		const res1 = await worker.fetch(req1, makeMockEnv());
		const res2 = await worker.fetch(req2, makeMockEnv());
		if (res1.status !== 200) {
	...
	it('rejects malformed JSON', async () => {
		const req = new Request('http://test', { method: 'POST', body: '{not valid json}' });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 400) {
			const text = await res.text();
			console.error('Test failed:', text);
	...
	it('accepts extra fields in payload', async () => {
		const req = new Request('http://test', { method: 'POST', body: JSON.stringify({ ...validSubmission, extraField: 'extra' }) });
		const res = await worker.fetch(req, makeMockEnv());
		if (res.status !== 200) {
			const text = await res.text();
			console.error('Test failed:', text);
